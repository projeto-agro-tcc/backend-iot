...
from backend_iot.enviroments import COLLECTIONS_EMW
...
class IotViewSet(ModelViewSet):
    ...
    def findbyparams(self, request, *args, **kwargs):
        ...
        colection = COLLECTIONS_EMW[request.query_params.get('var')]
        if (time_to_start and dev_id and colection) is not None:
            if time_to_end is None:
                time_to_end = datetime.now().timestamp()
            response = self.iot_service.getDataByParams(time_to_start, time_to_end, dev_id, colection)
